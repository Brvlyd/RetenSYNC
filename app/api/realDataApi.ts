    ...
    for (const user of users.slice(0, 20)) {
      ...
      for (let i = 0; i < numFeedback; i++) {
        ...
        feedback.push({
          ...
          project:
            template.projects[
              Math.floor(Math.random() * template.projects.length)
            ],
          content:
            template.content[
              Math.floor(Math.random() * template.content.length)
            ],
...
export const generateShoutoutData = async (): Promise<ShoutoutData[]> => {
  try {
    ...
    const shoutoutTemplates = [
      {
        message:
          'Amazing work on the project! Your dedication and expertise really made the difference.',
        values: ['Excellence', 'Teamwork'],
        emoji: '🌟',
      },
      {
        message:
          'Thank you for the quick turnaround on the bug fixes. Your commitment to quality is outstanding!',
        values: ['Customer Focus', 'Quality'],
        emoji: '🚀',
      },
      {
        message:
          'Great job leading the team through a challenging project. Your leadership skills shine!',
        values: ['Leadership', 'Innovation'],
        emoji: '💪',
      },
      {
        message:
          'Your creative solution to the technical challenge was brilliant. Keep up the innovation!',
    ...
    for (let i = 0; i < Math.min(15, users.length); i++) {
      ...
      if (fromUser.id !== toUser.id) {
        const template =
          shoutoutTemplates[
            Math.floor(Math.random() * shoutoutTemplates.length)
          ];
        const date = new Date();
  ...
  try {
    ...
    const goalTemplates = [
      {
        ...
        description:
          'Enhance technical expertise through certification and hands-on projects',
        keyResults: [
    ...
    for (const user of users.slice(0, 10)) {
      ...
      for (let i = 0; i < numGoals; i++) {
        ...
        dueDate.setMonth(
          dueDate.getMonth() + Math.floor(Math.random() * 6) + 1
        );
    ...
    for (const user of users.slice(0, 8)) {
      ...
      for (let i = 0; i < numMeetings; i++) {
        ...
        const manager = users.find(
          u => u.position.includes('Manager') || u.position.includes('Lead')
        );
        ...
        meetings.push({
          ...
          time,
          agenda:
            agendaTemplates[Math.floor(Math.random() * agendaTemplates.length)],
          notes:
...
export const getFeedbackData = () =>
  getCachedData('feedback', generateFeedbackData);
export const getShoutoutData = () =>
  getCachedData('shoutouts', generateShoutoutData);
export const getGoalData = () => getCachedData('goals', generateGoalData);
export const getOneOnOneData = () =>
  getCachedData('meetings', generateOneOnOneData);
