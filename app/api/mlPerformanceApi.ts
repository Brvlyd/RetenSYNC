...
export const predictTurnoverRisk = async (
  employee_id: number
  ...
  try {
    ...
    if (response.status === 401) {
      throw new Error(
        'Authentication failed. Please check your API token or login again.'
      );
    }
    ...
    console.warn(
      'Failed to get prediction from real API, falling back to demo:',
      error
    );
...
const API_BASE_URL =
  'https://turnover-api-hd7ze.ondigitalocean.app/api/performance/';
...
const isUsingDemoMode = (): boolean => {
  ...
  return (
    !authInfo.isValid || !authInfo.token || authInfo.token.startsWith('demo-')
  );
};
