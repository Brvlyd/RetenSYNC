...
const DEMO_DEPARTMENTS = [
  {
    ...
    name: 'Engineering',
    description:
      'Software development and technical operations team responsible for building and maintaining our products.',
  ...
  },
  {
    ...
    description:
      'People operations, recruitment, employee development, and organizational culture management.',
    created_at: '2024-01-15T08:00:00Z',
    updated_at: '2024-01-15T08:00:00Z',
    employee_count: 5,
  },
  {
    ...
    name: 'Marketing',
    description:
      'Brand management, digital marketing, content creation, and customer acquisition strategies.',
    created_at: '2024-01-15T08:00:00Z',
    updated_at: '2024-01-15T08:00:00Z',
    employee_count: 8,
  },
  {
    ...
    name: 'Sales',
    description:
      'Business development, client relationships, revenue generation, and market expansion.',
    created_at: '2024-01-15T08:00:00Z',
    updated_at: '2024-01-15T08:00:00Z',
    employee_count: 12,
  },
  {
    ...
    name: 'Finance',
    description:
      'Financial planning, accounting, budgeting, and strategic financial analysis for the organization.',
    created_at: '2024-01-15T08:00:00Z',
    updated_at: '2024-01-15T08:00:00Z',
    employee_count: 4,
  },
...
const isUsingDemoMode = (): boolean => {
  ...
  return (
    !authInfo.isValid ||
    !authInfo.token ||
    authInfo.token === 'demo-token' ||
    authInfo.token.startsWith('demo-')
  );
};
...
export const getDepartments = async (
  ...
  try {
    ...
    console.warn(
      'Failed to fetch from real API, falling back to demo data:',
      error
    );
    return {
      count: DEMO_DEPARTMENTS.length,
...
export const createDepartment = async (
  ...
  try {
    ...
    console.warn(
      'Failed to create via real API, falling back to demo mode:',
      error
    );
    const newDepartment: Department = {
      ...departmentData,
...
export const updateDepartment = async (
  ...
  try {
    ...
    console.warn(
      'Failed to update via real API, falling back to demo mode:',
      error
    );
    const index = DEMO_DEPARTMENTS.findIndex(dept => dept.id === id);
    if (index === -1) {
...
export const deleteDepartment = async (id: number): Promise<void> => {
  ...
  try {
    ...
    console.warn(
      'Failed to delete via real API, falling back to demo mode:',
      error
    );
    const index = DEMO_DEPARTMENTS.findIndex(dept => dept.id === id);
    if (index === -1) {
...
export const getDepartmentEmployees = async (
  ...
  try {
    ...
    console.warn(
      'Failed to fetch employees from real API, falling back to demo data:',
      error
    );
    return DEMO_EMPLOYEES.filter(emp => emp.department === id);
  }
