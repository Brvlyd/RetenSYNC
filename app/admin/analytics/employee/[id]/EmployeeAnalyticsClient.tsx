...
import { fetchPerformanceData, predictTurnoverRisk, PerformanceData } from '@/app/api/mlPerformanceApi';
...
export default function EmployeeAnalyticsClient({ id }: { id: string }) {
  ...
  const convertToEmployeeDetail = (data: PerformanceData): EmployeeDetail => {
    ...
    const engagement = Math.max(1, Math.min(10, (data.satisfaction_level * 10) + (data.promotion_last_5years ? 1 : 0))); // Enhanced by promotion
    const feedback = Math.max(1, Math.min(10, (data.last_evaluation * 10) + (data.number_project > 3 ? 1 : 0))); // Enhanced by projects
    ...
    if (!data.promotion_last_5years && data.time_spend_company >= 2) riskScore += 20;
  ...
  const generateRiskFactors = (employeeData: EmployeeDetail): RiskFactor[] => {
    ...
    if (!employeeData.promotionLast5Years && employeeData.timeSpendCompany >= 2) {
      factors.push({
    ...
    if (employeeData.workAccident) {
      factors.push({
        ...
        impact: 'medium',
        description: 'Has experienced a work-related accident, may affect morale and safety perception'
      });
    }
  ...
  const generateInterventions = (employeeData: EmployeeDetail, riskFactors: RiskFactor[]): Intervention[] => {
    const interventions: Intervention[] = [];
    
    // High priority interventions for high-risk factors
    const highRiskFactors = riskFactors.filter(f => f.impact === 'high');
    
    if (highRiskFactors.some(f => f.factor.includes('Satisfaction') || f.factor.includes('Engagement'))) {
      interventions.push({
        type: 'Engagement & Satisfaction',
        priority: 'urgent',
        action: 'Schedule immediate 1-on-1 discussion to understand concerns and create engagement plan',
    ...
    if (!employeeData.promotionLast5Years && employeeData.timeSpendCompany >= 2) {
      interventions.push({
        ...
        priority: 'urgent',
        action: 'Create clear career advancement roadmap and discuss promotion opportunities',
    ...
    if (employeeData.lastEvaluation < 0.5) {
      interventions.push({
        ...
        priority: 'high',
        action: 'Develop performance improvement plan with clear goals and support',
    ...
    if (employeeData.numberOfProjects > 6) {
      interventions.push({
        ...
        priority: 'medium',
        action: 'Review and prioritize project portfolio, delegate or postpone non-critical projects',
        timeline: 'Within 2 weeks',
        owner: 'Project Manager'
      });
    }
    ...
    if (employeeData.workAccident) {
      interventions.push({
        ...
        priority: 'medium',
        action: 'Provide additional safety training and psychological support if needed',
    ...
    interventions.push({
      ...
      priority: 'medium',
      action: 'Provide access to relevant training programs and professional development opportunities',
      timeline: 'Within 1 month',
  ...
  useEffect(() => {
    const fetchEmployeeData = async () => {
      try {
        ...
        const targetEmployee = performanceData.find(emp => emp.employee === parseInt(id));
        ...
        const interventions = generateInterventions(employeeDetail, riskFactors);
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <div className="bg-yellow-100 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-6 max-w-md mx-auto">
            ...
            <h3 className="text-lg font-semibold text-yellow-800 dark:text-yellow-200 mb-2">Employee Not Found</h3>
            <p className="text-yellow-600 dark:text-yellow-400 mb-4">The requested employee could not be found.</p>
            <button
              onClick={() => router.back()}
  ...
  return (
    <div className={`space-y-6 lg:space-y-8 pt-8 md:pt-12 xl:pt-16 p-3 sm:p-4 md:p-6 ${pageTopMargin}`}>
            ...
            <div className="border border-gray-200 dark:border-gray-700 rounded-xl p-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-xl font-bold text-gray-900 dark:text-white">{employee.numberOfProjects}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Monthly Hours</p>
                  <p className="text-xl font-bold text-gray-900 dark:text-white">{employee.averageMonthlyHours}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Last Evaluation</p>
                  <p className="text-xl font-bold text-gray-900 dark:text-white">{(employee.lastEvaluation * 100).toFixed(1)}%</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Time in Company</p>
                  <p className="text-xl font-bold text-gray-900 dark:text-white">{employee.timeSpendCompany} years</p>
                </div>
              </div>
            </div>
            
            <div className="border border-gray-200 dark:border-gray-700 rounded-xl p-4">
              <p className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-3">Status Indicators</p>
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-700 dark:text-gray-300">Promotion (Last 5 Years)</span>
                  <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
          ...
          <div className="space-y-4">
            {riskFactors.length > 0 ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-600 dark:text-gray-400">No significant risk factors identified</p>
              </div>
            )}
          </div>
        </div>
