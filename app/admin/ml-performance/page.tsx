...
export default function MLPerformancePage() {
  ...
  const [editFormData, setEditFormData] =
    useState<CreatePerformanceData | null>(null);
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : 'Failed to create performance data';
      setError(errorMessage);
  ...
  const filteredData = performanceData.filter(
    item =>
      ...
      (item.employee_email?.toLowerCase() || '').includes(
        searchTerm.toLowerCase()
      ) ||
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to update performance data'
      );
      setSuccessMessage(null);
    }
  };
  ...
  const handleExportData = () => {
    ...
    try {
      ...
      link.setAttribute(
        'download',
        `ml-performance-data-${new Date().toISOString().split('T')[0]}.csv`
      );
      ...
      setError(null);
      setSuccessMessage(
        `Successfully exported ${filteredData.length} records to CSV file`
      );
      setTimeout(() => setSuccessMessage(null), 3000);
    } catch (err) {
  ...
  return (
    <div
      className={'min-h-screen bg-gray-50 dark:bg-gray-900 p-3 sm:p-4 md:p-6'}
    >
      <div
        className={`space-y-4 sm:space-y-6 lg:space-y-8 animate-fade-in ${pageTopMargin}`}
      >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-amber-700 dark:text-amber-300 text-sm">
                    No performance data available. Please add data or check your
                    API connection.
                  </p>
                </div>
              </div>
                    ...
                    {performanceData.length > 0
                      ? (
                        (performanceData.reduce(
                          (acc, curr) => acc + curr.satisfaction_level,
                          0
                        ) /
                            performanceData.length) *
                          100
                  ...
                  <p className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">
                    {
                      performanceData.filter(
                        emp => calculateRiskLevel(emp).level === 'High'
                      ).length
                    }
                  </p>
                </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">
                    {
                      performanceData.filter(emp => emp.promotion_last_5years)
                        .length
                    }
                  </p>
                </div>
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredData.length === 0 ? (
                    <tr>
                      <td
                        colSpan={8}
                        className="px-3 sm:px-6 py-8 text-center text-gray-500 dark:text-gray-400 text-sm"
                      >
                        {searchTerm
                    ...
                    filteredData.map(employee => {
                      ...
                      return (
                        ...
                        >
                            ...
                            <div className="flex items-center">
                              <div className="w-12 sm:w-16 bg-gray-200 rounded-full h-2 mr-2">
                                <div
                                  ...
                                  style={{
                                    width: `${employee.satisfaction_level * 100}%`,
                                  }}
                                ></div>
                              </div>
                            ...
                            <div className="flex items-center">
                              <div className="w-12 sm:w-16 bg-gray-200 rounded-full h-2 mr-2">
                                <div
                                  ...
                                  style={{
                                    width: `${employee.last_evaluation * 100}%`,
                                  }}
                                ></div>
                              </div>
                              <span className="text-xs sm:text-sm text-gray-900 dark:text-white">
                          ...
                          <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                            <span
                              className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${risk.bg} ${risk.color}`}
                            >
                              {risk.level}
                            </span>
                          </td>
                      ...
                      <label className="flex items-center">
                        <input
                          ...
                          onChange={e =>
                            setCreateFormData({
                              ...createFormData,
                              work_accident: e.target.checked,
                            })
                          }
                          className="mr-2"
                        />
                      ...
                      <label className="flex items-center">
                        <input
                          ...
                          onChange={e =>
                            setCreateFormData({
                              ...createFormData,
                              promotion_last_5years: e.target.checked,
                            })
                          }
                          className="mr-2"
                        />
                ...
                {!editMode ? (
                  <>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm sm:text-base text-gray-900 dark:text-white">
                          {(selectedEmployee.satisfaction_level * 100).toFixed(
                            1
                          )}
                          %
                        </p>
                      ...
                      <div>
                        ...
                        <p className="text-sm sm:text-base text-gray-900 dark:text-white">
                          {selectedEmployee.number_project}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
                        ...
                        <p className="text-sm sm:text-base text-gray-900 dark:text-white">
                          {selectedEmployee.average_monthly_hours}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
                        ...
                        <p className="text-sm sm:text-base text-gray-900 dark:text-white">
                          {selectedEmployee.time_spend_company} years
                        </p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
                        ...
                        <p className="text-sm sm:text-base text-gray-900 dark:text-white">
                          {selectedEmployee.work_accident ? 'Yes' : 'No'}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
                        ...
                        <p className="text-sm sm:text-base text-gray-900 dark:text-white">
                          {selectedEmployee.promotion_last_5years
                            ? 'Yes'
                            : 'No'}
                        </p>
                      </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                      ...
                      >
                        {predicting ? (
                          ...
                          <>
                            <Brain className="h-4 w-4 sm:h-5 sm:w-5" /> Predict
                            ML
                          </>
                        )}
                      </button>
                      ...
                      <div className="mt-4 p-3 sm:p-4 bg-gray-100 dark:bg-gray-700 rounded text-sm sm:text-base text-gray-900 dark:text-white">
                        {predictionResult.error ? (
                          ...
                          <>
                              ...
                              {predictionResult.prediction?.risk_level ? (
                                ...
                                >
                                  {predictionResult.prediction.risk_level
                                    .charAt(0)
                                    .toUpperCase() +
                                    predictionResult.prediction.risk_level.slice(
                                      1
                                    )}
                                </span>
                              ) : (
                            ...
                            <div className="mb-2">
                              <span className="font-semibold">Will Leave:</span>{' '}
                              {predictionResult.prediction?.will_leave === true
                                ? 'Yes'
                                : predictionResult.prediction?.will_leave ===
                                    false
                                  ? 'No'
                                  : '-'}
                            </div>
                            <div className="mb-2">
                              ...
                              {typeof predictionResult.prediction
                                ?.probability === 'number'
                                ? (
                  ...
                  >
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          min="0"
                          max="1"
                          value={editFormData?.satisfaction_level ?? ''}
                          onChange={e =>
                            setEditFormData(f =>
                              f
                                ? {
                                  ...f,
                                  satisfaction_level: parseFloat(
                                    e.target.value
                                  ),
                                }
                                : f
                      ...
                      <div>
                        ...
                        <input
                          ...
                          min="0"
                          max="1"
                          value={editFormData?.last_evaluation ?? ''}
                          onChange={e =>
                            setEditFormData(f =>
                              f
                                ? {
                                  ...f,
                                  last_evaluation: parseFloat(e.target.value),
                                }
                                : f
                            )
                          }
                      ...
                      <div>
                        ...
                        <input
                          type="number"
                          min="0"
                          value={editFormData?.number_project ?? ''}
                          onChange={e =>
                            setEditFormData(f =>
                              f
                                ? {
                                  ...f,
                                  number_project: parseInt(e.target.value),
                                }
                                : f
                            )
                          }
                      ...
                      <div>
                        ...
                        <input
                          type="number"
                          min="0"
                          value={editFormData?.average_monthly_hours ?? ''}
                          onChange={e =>
                            setEditFormData(f =>
                              f
                                ? {
                                  ...f,
                                  average_monthly_hours: parseInt(
                                    e.target.value
                                  ),
                                }
                                : f
                      ...
                      <div>
                        ...
                        <input
                          type="number"
                          min="0"
                          value={editFormData?.time_spend_company ?? ''}
                          onChange={e =>
                            setEditFormData(f =>
                              f
                                ? {
                                  ...f,
                                  time_spend_company: parseInt(
                                    e.target.value
                                  ),
                                }
                                : f
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 col-span-full">
                        <label className="flex items-center">
                          <input
                            type="checkbox"
                            checked={!!editFormData?.work_accident}
                            onChange={e =>
                              setEditFormData(f =>
                                f
                                  ? { ...f, work_accident: e.target.checked }
                                  : f
                              )
                            }
                            className="mr-2"
                        ...
                        <label className="flex items-center">
                          <input
                            type="checkbox"
                            checked={!!editFormData?.promotion_last_5years}
                            onChange={e =>
                              setEditFormData(f =>
                                f
                                  ? {
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mt-4">
                      <button
                        type="submit"
                        className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-2 px-4 text-sm sm:text-base rounded-lg transition-colors"
                      >
                        Save
                      </button>
                      <button
                        ...
                        className="flex-1 bg-gray-300 hover:bg-gray-400 text-gray-700 py-2 px-4 text-sm sm:text-base rounded-lg transition-colors"
                      >
                        Cancel
                      </button>
                    </div>
