...
export function useDepartments() {
  ...
  const fetchDepartments = useCallback(
    async (page: number = 1, pageSize: number = 10) => {
      ...
      try {
        const response: DepartmentListResponse = await getDepartments(
          page,
          pageSize
        );
  ...
  const createNewDepartment = useCallback(
    async (
  ...
  const updateExistingDepartment = useCallback(
    async (
      id: number,
  ...
  // Delete a department
  const deleteExistingDepartment = useCallback(
